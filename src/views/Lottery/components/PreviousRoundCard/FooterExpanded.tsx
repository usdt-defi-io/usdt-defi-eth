...
const PreviousRoundCardFooter: React.FC<{ lotteryNodeData: LotteryRound; lotteryId: string }> = ({
  ...
  if (lotteryNodeData) {
    const { amountCollectedInDexToken } = lotteryNodeData
    prizeInBusd = amountCollectedInDexToken.times(cakePriceBusd)
  }
      ...
      <>
        ...
        {prizeInBusd.isNaN() ? (
          ...
          <Balance
            ...
            color="textSubtle"
            unit=" USDTd"
            value={getBalanceNumber(lotteryNodeData?.amountCollectedInDexToken)}
            decimals={0}
          />
        )}
      </>
