...
import { useETHBusdPrice } from 'hooks/useBUSDPrice'
...
const SetPriceStage: React.FC<SetPriceStageProps> = ({
  ...
  const bnbPrice = useETHBusdPrice()
  ...
  const { tooltip, tooltipVisible, targetRef } = useTooltip(
    <>
      <Text>
        {t(
          'When selling NFTs from this collection, a portion of the ETH paid will be diverted before reaching the seller:',
        )}
      </Text>
      {creatorFeeAsNumber > 0 && (
        <Text>{t('%percentage%% royalties to the collection owner', { percentage: creatorFee })}</Text>
      )}
      <Text>{t('%percentage%% trading fee will be used to buy & burn USDTd', { percentage: tradingFee })}</Text>
    </>,
    { placement: 'auto' },
  )
    ...
    <>
      ...
      <GreyedOutContainer>
        ...
        {priceIsOutOfRange && (
          <Text fontSize="12px" color="failure">
            {t('Allowed price range is between %minPrice% and %maxPrice% WETH', {
              minPrice: MIN_PRICE,
              maxPrice: MAX_PRICE,
            })}
          </Text>
      ...
      <Grid gridTemplateColumns="32px 1fr" p="16px" maxWidth="360px">
        ...
        <Box>
          ...
          <Text small color="textSubtle">
            {t('Sales are in WETH. You can swap WETH to ETH 1:1 for free with USDT DeFi.')}
          </Text>
        </Box>
      </Grid>
      <Divider />
