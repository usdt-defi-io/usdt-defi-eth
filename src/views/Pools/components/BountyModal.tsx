...
const BountyModal: React.FC<BountyModalProps> = ({ onDismiss, TooltipComponent }) => {
  ...
  const dexTokenVaultContract = useCakeVaultContract()
  ...
  const totalYieldToDisplay = getBalanceNumber(totalpendingDexTokenHarvest, 18)
  ...
  const handleConfirmClick = async () => {
    setPendingTx(true)
    try {
      const tx = await callWithGasPrice(dexTokenVaultContract, 'harvest', undefined, { gasLimit: 300000 })
      const receipt = await tx.wait()
      if (receipt.status) {
        toastSuccess(
          t('Bounty collected!'),
          <ToastDescriptionWithTx txHash={receipt.transactionHash}>
            {t('USDTd bounty has been sent to your wallet.')}
    ...
    <Modal title={t('Claim Bounty')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <Flex alignItems="flex-start" justifyContent="space-between">
        ...
        <Flex flexDirection="column">
          <Balance bold value={cakeBountyToDisplay} decimals={7} unit=" USDTd" />
      ...
      <Flex alignItems="center" justifyContent="space-between">
        ...
        <Balance color="textSubtle" value={totalYieldToDisplay} unit=" USDTd" />
      </Flex>
