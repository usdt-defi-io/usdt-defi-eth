...
const ActionPanel: React.FC<ActionPanelProps> = ({ account, pool, userDataLoaded, expanded, breakpoints }) => {
  ...
  const dexTokenVaultContractAddress = getCakeVaultAddress()
    ...
    <StyledActionPanel expanded={expanded}>
      <InfoSection>
        ...
        {poolContractAddress && (
          <Flex mb="8px" justifyContent={['flex-end', 'flex-end', 'flex-start']}>
            <LinkExternal
              href={`${BASE_BSC_SCAN_URL}/address/${isAutoVault ? dexTokenVaultContractAddress : poolContractAddress}`}
              bold={false}
            >
              {t('View Contract')}
            </LinkExternal>
      ...
      <ActionContainer>
        {showSubtitle && (
          <Text mt="4px" mb="16px" color="textSubtle">
            {isAutoVault ? t('Automatic restaking') : `${t('Earn')} USDTd ${t('Stake').toLocaleLowerCase()} USDTd`}
          </Text>
        )}
        {pool.isAutoVault ? (
