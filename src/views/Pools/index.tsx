...
const Pools: React.FC = () => {
  ...
  const {
    userData: { dexTokenAtLastUserAction, userShares },
    fees: { performanceFee },
    pricePerFullShare,
      ...
      case 'totalStaked':
        return orderBy(
          ...
          (pool: DeserializedPool) => {
            ...
            if (pool.isAutoVault) {
              if (pool.stakingTokenPrice && totalDexTokenInVault.isFinite()) {
                totalStaked =
                  +formatUnits(ethers.BigNumber.from(totalDexTokenInVault.toString()), pool.stakingToken.decimals) *
                  pool.stakingTokenPrice
              }
            } else if (pool.sousId === 0) {
              if (pool.totalStaked?.isFinite() && pool.stakingTokenPrice && totalDexTokenInVault.isFinite()) {
                const manualCakeTotalMinusAutoVault = ethers.BigNumber.from(pool.totalStaked.toString()).sub(
                  totalDexTokenInVault.toString(),
                )
                totalStaked =
