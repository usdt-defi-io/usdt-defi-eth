...
export const transformBetResponse = (betResponse: BetResponse): Bet => {
  const bet = {
    ...
    claimedETH: betResponse.claimedETH ? parseFloat(betResponse.claimedETH) : 0,
    claimedNetETH: betResponse.claimedNetETH ? parseFloat(betResponse.claimedNetETH) : 0,
...
export const transformUserResponse = (userResponse: UserResponse): PredictionUser => {
  ...
  return {
    ...
    totalETH: totalETH ? parseFloat(totalETH) : 0,
    totalETHBull: totalETHBull ? parseFloat(totalETHBull) : 0,
    totalETHBear: totalETHBear ? parseFloat(totalETHBear) : 0,
    totalBetsClaimed: numberOrNull(totalBetsClaimed),
    totalETHClaimed: totalETHClaimed ? parseFloat(totalETHClaimed) : 0,
    winRate: winRate ? parseFloat(winRate) : 0,
    averageETH: averageETH ? parseFloat(averageETH) : 0,
    netETH: netETH ? parseFloat(netETH) : 0,
  }
}
...
export const getTotalWon = async (): Promise<number> => {
  ...
  const totalETH = market.totalETH ? parseFloat(market.totalETH) : 0
  const totalETHTreasury = market.totalETHTreasury ? parseFloat(market.totalETHTreasury) : 0

  return Math.max(totalETH - totalETHTreasury, 0)
}
