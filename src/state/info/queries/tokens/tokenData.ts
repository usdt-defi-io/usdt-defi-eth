    ...
    const fetch = async () => {
      ...
      if (error) {
        ...
        const formatted = tokenAddresses.reduce((accum: { [address: string]: TokenData }, address) => {
          ...
          const priceUSD = current ? current.derivedETH * bnbPrices.current : 0
          const priceUSDOneDay = oneDay ? oneDay.derivedETH * bnbPrices.oneDay : 0
          const priceUSDWeek = week ? week.derivedETH * bnbPrices.week : 0
